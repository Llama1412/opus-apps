...
local args = { ... }
local options = { chunks = { arg = 'c', type = 'number', value = -1, desc = 'Number of chunks to mine' }, setTrash = { arg = 's', type = 'flag', value = false, desc = 'Add ignore items' }, help = { arg = 'h', type = 'flag', value = false, desc = 'Displays the options' }, }
...
local mining
local ignores = { ignore = true, retain = true, }

local dictionary = { data = Util.readTable(DICTIONARY_FILE) or { ['minecraft:chest'] = 'suck', ['minecraft:lava'] = 'liquid_fuel', ['minecraft:flowing_lava'] = 'liquid_fuel', ['minecraft:mob_spawner'] = 'ignore', ['minecraft:bedrock'] = 'ignore', ['minecraft:flowing_water'] = 'ignore', ['minecraft:water'] = 'ignore', ['minecraft:air'] = 'ignore', ['minecraft:bucket'] = 'retain', ['computercraft:wireless_modem_advanced'] = 'retain', ['minecraft:diamond_pickaxe'] = 'retain', ['advancedperipherals:geo_scanner'] = 'retain', }, }
...
local page = UI.Page { menuBar = UI.MenuBar { buttons = {
    --{ text = 'Mine',   event = 'mine' },
    { text = 'Ignore', event = 'ignore' }, { text = 'Abort', event = 'abort', x = -8 }, }, }, grid = UI.Grid { y = 2, ey = -2, sortColumn = 'name', columns = { { heading = 'Count', key = 'count', width = 5, align = 'right' }, { heading = 'Resource', key = 'displayName' }, }, }, statusBar = UI.StatusBar { columns = { { key = 'status' }, { key = 'distance', width = 4 }, { key = 'fuel', width = 5 }, }, }, accelerators = { ['control-q'] = 'cancel', } }
...
local function getChunkCoordinates(diameter, index, x, z)
    local dirs = { -- circumference of grid
        ...
        { xd = -1, zd = 0, heading = 2 }, { xd = 0, zd = -1, heading = 3 }, { xd = 1, zd = 0, heading = 0 }  -- east
    }
...
local function scan()
    ...
    local blocks
    repeat blocks = scanner.scan(8) until blocks
    ...
    Util.filterInplace(blocks, function(b)
        ...
        if b.y >= bedrock then
            ...
            if not counts[key] then
                counts[key] = { displayName = key, name = b.name, damage = 0, count = 1 }
            else
...
local function mineChunk()
    ...
    Point.eachClosest(turtle.point, pts, function(pt)
        ...
        local chunks = math.pow(mining.diameter - 2, 2) + mining.chunkIndex
        status(string.format('scanning %d %d-%d', chunks, pt.y + mining.home.y - 8, pt.y + mining.home.y + 8))
...
turtle.initialize = function()
    ...
    elseif pt.z > pt2.z then
        ...
        error('Coordinates did not change (fuel??)')
    end
...
local function main()
    repeat mineChunk() until not nextChunk()
end
...
if not fs.exists(STARTUP_FILE) then
    Util.writeFile(STARTUP_FILE, [[print('sleeping for 10 seconds')
os.sleep(10)
...
Event.addRoutine(function()
    ...
    turtle.initialize { right = 'computercraft:wireless_modem_advanced', left = 'minecraft:diamond_pickaxe', required = { 'minecraft:bucket', 'advancedperipherals:geo_scanner', }, GPS = true, minFuel = 100, -- searchFor = 'ironchest:iron_shulker_box_white'
    }
    ...
    mining = Util.readTable(PROGRESS_FILE) or { diameter = 1, chunkIndex = 0, x = 0, z = 0, chunks = options.chunks.value, home = Point.copy(turtle.point), heading = turtle.point.heading, -- always using east for now
    }
    ...
    turtle.setPoint({ x = turtle.point.x - mining.home.x, y = turtle.point.y - mining.home.y, z = turtle.point.z - mining.home.z, })

    local distance = Point.distance({ x = turtle.point.x, y = 0, z = turtle.point.z }, HOME_PT)
    local maxDistance = Point.distance(HOME_PT, { x = mining.x, y = 0, z = mining.z })
    ...
    turtle.set({ attackPolicy = 'attack', digPolicy = 'turtleSafe', movementStrategy = 'goto', })
    status('mining')
